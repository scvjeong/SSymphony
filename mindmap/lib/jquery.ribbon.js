...
(function($) {
    ...
    $.fn.Ribbon = function(ribbonSettings) {
        ...
        $('head').append('<link href="images/ribbon/themes/' + settings.theme + '/ribbon.css" rel="stylesheet" type="text/css" />" />');
        ...
        function SetupMenu(settings) {
            ...
            $('.ribbon-list div').parents().click(function(e) {
				...
				if( $('.ribbon-list div ul:visible').parent().offset() ) {
					outsideX = e.pageX < $('.ribbon-list div ul:visible').parent().offset() || e.pageX > $('.ribbon-list div ul:visible').parent().offset().left + $('.ribbon-list div ul:visible').parent().width();
					outsideY = e.pageY < $('.ribbon-list div ul:visible').parent().offset() || e.pageY > $('.ribbon-list div ul:visible').parent().offset().top + $('.ribbon-list div ul:visible').parent().height();
				}
        ...
        $('.orb li:first ul:first').prepend('<img src="images/ribbon/themes/' + settings.theme + '/images/menu_top.png" style="margin-left: -10px; margin-top: -22px;" />');
        $('.orb li:first ul:first').append('<img src="images/ribbon/themes/' + settings.theme + '/images/menu_bottom.png" style="margin-left: -10px; margin-bottom: -22px;" />');

        $('.ribbon-list div').each(function() { if ($(this).children('ul').length > 0) { $(this).append('<img src="images/ribbon/themes/' + settings.theme + '/images/arrow_down.png" style="float: right; margin-top: 5px;" />') } });
