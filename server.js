...
var meeting = io.of('/group').on('connection', function (socket) {
	...
	socket.on('set_insert_data', function(data) {
		...
		client.set(storeId, tmpVal);	 //key에 데이터 저장
		...
		client.llen(tmpOrder, function (err, idVal) {			
			client.lindex(tmpOrder, tmpIndex, function (err, preId) {
				...
				socket.broadcast.to(tmpGroup).emit('get_insert_data', {tool: tmpTool, id: tmpId, index: tmpIndex, val: tmpVal });
			});	
		});
	});
	...
	socket.on('set_insert_tree_data', function(data) {
		...
		client.hlen(storeId, function (err, num) {
			if ( num > 0 )
			{
				client.hkeys(storeId, function (err, parent) {
					client.hdel(storeId, parent);	
					client.hset(storeId, storeParent, tmpVal);	 //hash에 데이터 저장
				});
			}
			else
			{
		...
		client.llen(tmpOrder, function (err, idVal) {			
			client.lindex(tmpOrder, tmpIndex, function (err, preId) {
				...
				socket.broadcast.to(tmpGroup).emit('get_insert_tree_data', { tool: tmpTool, id: tmpId, parent: tmpParent, index: tmpIndex, val: tmpVal });
			});	
		});
	});
