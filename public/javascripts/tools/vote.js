...
function initVote(group, tool)
{
	...
	_socket_vote.emit('join_room', {idx_meeting: _idx_meeting});						// 그룹에 join
	_socket_vote.emit('set_vote_data', {idx_meeting: _idx_meeting, tool: tool});		// 서버에 초기 데이터 요청
	//_socket_vote.emit('set_client', {idx_meeting: _idx_meeting, user: 'user1'});
	_socket_vote.emit('set_tree_option_data', {idx_meeting: _idx_meeting, tool: tool, option: 'is_multi_vote'});
...
function startVote()
{
	...
	_socket_vote.emit('set_option_data', {idx_meeting: _idx_meeting, tool: _voteid,
	...
	for (var i = 0; i < listcount; i++)
	{
		...
		if (this_title != "")
		{
			...
			console.log(i+1);
			_socket_vote.emit('set_insert_vote_data', { idx_meeting: _idx_meeting, tool: _voteid, id: _tmpLastId, parent: i+1, index: 1, val: this_title, client: _client_id } );
		}
	}
...
*/
	...
	_socket_vote.on('get_last_id', function (data) { 
		...
		_socket_vote.emit('set_insert_vote_data', { idx_meeting: _idx_meeting, tool: _voteid, id: _tmpLastId, parent: '0', index: '0', val: title_val, client: _client_id} );
		startVote();
	});
...
function sendVoteDataToServer()
{
	...
	_socket_vote.emit('set_voting_data', {idx_meeting: _idx_meeting, tool: _voteid,
